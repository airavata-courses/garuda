...
function checkRequestIdInsertOrReturn(receivedData, res) {
  ...
  userRequestsModel.find({ request_id: paramRequestId }, (err, data) => {
    if (!err) {
      ...
      if (data.length != 0) {
        res.send({ "status": "success", "message": "Information retrieved", "requests": data, "is_new_request": "false" });
...
function checkIfRequestIdExists(receivedData, res) {
  ...
  userRequestsModel.find({ request_id: paramRequestId, property: paramPropertyType }, (err, data) => {
    if (!err) {
      //data: array of objects
      if (data.length != 0) {
        res.send({ "status": "success", "message": "Information retrieved", "requests": data, "data_status": "true" });
        //Below code acts as a caching mechanism if the request made by the user already exists in DB but the initial request was made by some other user then insert the same entry with new user's user email
        //TODO: The above fix causes a bug i.e the 'data' variable which is returned has multiple entries of same request and email id of other user is getting exposed
        ...
        res.send({ "status": "success", "message": "Information not present", "data_status": "false" });
      }
    } else {
      res.send({ "status": "error", "message": "Retrieval failed", "data_status": "false" });
    }
  }
  );
}
...
function insertUserRequest(objUserRequest, response) {
  ...
  userRequestsModel.find({ request_id: paramRequestId, property:paramPropertyType , user_email:paramUserEmail }, (err, data) => {
      ...
      if(data.length == 0){
        ...
        oUserReq.save((err) => {
          if (!err) {
            ...
            response.send({ "status": "success", "message": "Insertion successful"});
          }
          else {
...
function getAllUserRequests(receivedData, res){
  userRequestsModel.find({ user_email: receivedData.user_email }, (err, data) => {
    if (!err) {
      //data: array of objects
      res.send({ "status": "success", "message": "Information retrieved", "requests": data });
    } else {
      res.send({ "status": "error", "message": "Retrieval failed" });
    }
  });
...
function getDataOfRequestId(receivedData, res){
  dataSetModel.find({ request_id: receivedData.request_id, property: receivedData.property }, (err,data) => {
    if(!err){
      //data: array of objects
      console.log(data)
      res.send({ "status": "success", "message": "Information retrieved", "data": data });
    } else {
      res.send({ "status": "error", "message": "Retrieval failed" });
    }
  })
...
function updateStatusOfRequestInDB(oDataSetRequest, response) {
  userRequestsModel.updateOne({ request_id: oDataSetRequest.request_id }, { status: CONSTANTS.CONST_REQUEST_STATUS_COMPLETE }, function (err, docs) {
    if (!err) {
      // docs response
      // { 
      //   acknowledged: true,
      ...
      if (docs.modifiedCount && docs.matchedCount) {
        response.send({ "status": "success", "message": "Insert and Update successful" });
      } else {
        response.send({ "status": "error", "message": "Something went wrong during the update operation" });
      }
      
    } else {
      response.send({ "status": "error", "message": "Update failed in user request collection" });
    }
  }
  )
}
